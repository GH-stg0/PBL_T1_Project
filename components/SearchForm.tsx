...
export default function SearchForm() {
  ...
  // ▼ カテゴリー: 配列で管理（元の大元コードそのまま）
  const [categories, setCategories] = useState<string[]>([]);

  // ▼ 発見場所: セレクトボックスの値をステートで保持（新規追加）
  ...
  return (
    <div className="space-y-6 text-gray-700">
      {/* カテゴリーセクション（大元コードの CategoryInput を利用） */}
      <section>
        <CategoryInput
      ...
      <section>
        ...
        <div className="relative">
          ...
          <select
            className="w-full appearance-none bg-app-input border border-gray-400 rounded-full py-2 pl-10 pr-10 text-gray-700 focus:outline-none"
            value={location}
      ...
      <section>
        ...
        <div className="relative w-full">
          ...
          <div className="w-full flex items-center bg-app-input border border-gray-400 rounded-full py-2 px-3 text-gray-600 shadow-sm pointer-events-none">
            ...
            <span>
              {dateStr ? dateStr.replace(/-/g, "/") : "カレンダーから選択"}
            </span>
          </div>
        </div>
      </section>
